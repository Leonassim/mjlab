#!/usr/bin/env bash
# Point de convergence TOUTES les heures, en boucle. La version precedente
# dormait 3600 s puis rendait la main : un seul controle au lieu d'une veille.
set -u
cd /home/lmoussafir/mjlab-rhps1 || exit 1
for round in $(seq 1 "${2:-12}"); do
  sleep "${1:-3600}"
  RUN=$(ls -1dt logs/rsl_rl/rhps1_velocity/*/ | head -1)
  echo "===== controle $round  $(date +%m-%d_%H:%M) ====="
...
print(f"{'':<9}{'-2000':>10}{'-1000':>10}{'derive%':>9}")
plat=True
for n in T:
    x=win(n,last-2000,last-1000); y=win(n,last-1000,last)
    dr=100*(y-x)/abs(x) if x and x==x and abs(x)>1e-9 else float('nan')
    if n in ("track","clear","steplen") and dr==dr and abs(dr)>2.0: plat=False
    print(f"{n:<9}{x:10.4f}{y:10.4f}{dr:9.1f}")
print("VERDICT:", "PLATEAU" if plat else "progresse encore")
PY
done
